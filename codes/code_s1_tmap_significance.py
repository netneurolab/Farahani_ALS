...
Files to Load:
    'w_score_visit1HC_visit' + str(timepoint) + 'ALS' + str(filename) + '.npy'

Notes:
    The w-score is raw and not yet multiplied by -1.
...
subtype = 'all' # run it with "all", "spinal", and "bulbar"
nnodes  = 400
...
np.save(os.path.join(path_results, f't_statistics_cortex_{subtype}_schaefer_{nnodes}.npy'), t_statistic_cortex)

t_statistic_subcortex = calculate_t_statistic(w_score, parcel_indices_JHU)
np.save(os.path.join(path_results, f't_statistics_subcortex_{subtype}.npy'), t_statistic_subcortex)
...
plt.savefig(os.path.join(path_fig, 'sig_values_subcortex_' + subtype + '.png'), dpi = 300)
plt.close()
...
# Sorting data to create a barplot 
